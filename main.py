...
with col2:
    # Use a plain placeholder for showing the translation (direct approach)
    translation_placeholder = st.empty()
    translation_placeholder.text_area("Translation:", value=st.session_state.translation, height=None, max_chars=None, key="translation_output")
...
if st.button('Translate Sentence'):
    if text == "":
        ...
        try:
            # Using deep_translator instead of google_trans_new
            translator = GoogleTranslator(source=value1, target=value2)
            translated_text = translator.translate(text)
            
            # Update the translation directly on the page
            st.session_state.translation = translated_text
            # Update the text area immediately with the translation
            translation_placeholder.text_area("Translation:", value=translated_text, height=None, max_chars=None, key="translation_output")
            
            # Error handling for TTS
            try:
                converted_audio = gtts.gTTS(translated_text, lang=value2)
                ...
                st.success("Translation completed, but audio generation failed.")
                
        except Exception as e:
            st.error(f"Translation failed: {str(e)}")
            st.warning("Try another language pair or check your internet connection.")
else:
    pass