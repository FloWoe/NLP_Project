...
class TestVocabStorageFlow(unittest.TestCase):
    ...
    def test_save_and_read_vocab_entry(self):
        ...
        self.assertEqual(db_entry[1], entry.original_word)
        self.assertEqual(db_entry[2], entry.translated_word)
        self.assertEqual(db_entry[3], entry.source_lang)
        self.assertEqual(db_entry[4], entry.target_lang)
        self.assertEqual(db_entry[5], entry.original_sentence)
        self.assertEqual(db_entry[6], entry.translated_sentence)
...
# 🔽 Macht den Direktstart per `python test_functional_flow.py` möglich
